import { getDelivery, getItems, getItemsOffset } from "./lightspeed";
...
export async function getInventory() {
  ...
  const products = items.map((item) => {
    return {
      ...
      image: item.Images
        ? `${item.Images.Image.baseImageURL}/w_250/${item.Images.Image.publicID}.jpg`
        : null,
      currency: "GBP",
    };
  });
...
export async function sendEmailConfirmation(stripeSessionData, saleID) {
  const emailLineItems = stripeSessionData.line_items.data.map((line) => {
    return `<p><strong>Description:</strong> ${line.description} - <strong>Qty:</strong> ${line.quantity}</p>`;
  });
